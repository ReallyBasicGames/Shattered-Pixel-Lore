...
public abstract class Char extends Actor {
	...
	public boolean attack( Char enemy, float dmgMulti, float dmgBonus, float accMulti ) {
		...
		if (enemy.isInvulnerable(getClass())) {
			...
			SlimeArmor slime = enemy.buff(SlimeArmor.class);
			if (slime != null)   dr += Random.NormalIntRange( slime.level()/2 , slime.level() );
			...
			if (this instanceof Hero){
				...
				if(h.hasTalent(Talent.CAUSTIC_OOZE)) {
					if (Random.Int( 10 ) <= h.pointsInTalent(Talent.CAUSTIC_OOZE)) {
